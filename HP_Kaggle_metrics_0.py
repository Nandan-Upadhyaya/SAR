...
class CycleGAN:
    def __init__(
        ...
        self.discriminator_loss = tf.keras.losses.MeanSquaredError()
...
val_ds: tf.data.Dataset,
        ...
        # Initialize metrics
         ...
         self.metrics_calculator = tf.keras.metrics.Mean()  # Initialize a default metric calculator
    ...
    def distributed_train_step(self, inputs):
        """Fixed distributed training step"""
        def _replica_step(inputs):
            ...
            with tf.GradientTape(persistent=True) as tape:
                ...
                disc_real_optical_adv, disc_real_optical_cls = self.model.D1(opt_img)
                disc_fake_optical_adv, disc_fake_optical_cls = self.model.D1(fake_optical)
                disc_real_sar_adv, disc_real_sar_cls = self.model.D2(sar_img)
                disc_fake_sar_adv, disc_fake_sar_cls = self.model.D2(fake_sar)

                # Generator losses
                gen_loss_optical = tf.reduce_mean(tf.square(disc_fake_optical_adv - 1.0))
                gen_loss_sar = tf.reduce_mean(tf.square(disc_fake_sar_adv - 1.0))
                gen_adv_loss = gen_loss_optical + gen_loss_sar

                # Cycle consistency loss
                cycle_loss = (tf.reduce_mean(tf.abs(sar_img - cycled_sar)) +
                            tf.reduce_mean(tf.abs(opt_img - cycled_optical))) * self.lambda_cyc

                # Classification losses
                cls_loss_G1 = self.model._classification_loss(opt_mask, disc_fake_optical_cls)
                cls_loss_G2 = self.model._classification_loss(sar_mask, disc_fake_sar_cls)
                cls_loss_G = (cls_loss_G1 + cls_loss_G2) * self.lambda_cls

                # Total generator loss
                total_gen_loss = gen_adv_loss + cycle_loss + cls_loss_G

                # Discriminator losses
                disc_loss_optical = tf.reduce_mean(tf.square(disc_real_optical_adv - 1.0) + 
                                                 tf.square(disc_fake_optical_adv))
                disc_loss_sar = tf.reduce_mean(tf.square(disc_real_sar_adv - 1.0) + 
                                             tf.square(disc_fake_sar_adv))
                disc_adv_loss = (disc_loss_optical + disc_loss_sar) * 0.5

                # Classification losses for discriminators
                cls_loss_D1 = self.model._classification_loss(opt_mask, disc_real_optical_cls)
                cls_loss_D2 = self.model._classification_loss(sar_mask, disc_real_sar_cls)
                cls_loss_D = (cls_loss_D1 + cls_loss_D2) * self.lambda_cls
            ...
            # Calculate gradients
            gen_vars = self.model.G1.trainable_variables + self.model.G2.trainable_variables
            disc_vars = self.model.D1.trainable_variables + self.model.D2.trainable_variables

            gen_gradients = tape.gradient(total_gen_loss, gen_vars)
            disc_gradients = tape.gradient(total_disc_loss, disc_vars)

            # Apply gradients
            self.model.gen_optimizer.apply_gradients(zip(gen_gradients, gen_vars))
            self.model.disc_optimizer.apply_gradients(zip(disc_gradients, disc_vars))

            # Calculate metrics
            psnr = self.metrics_calculator.calculate_psnr(opt_img, fake_optical)
            ssim = self.metrics_calculator.calculate_ssim(opt_img, fake_optical)
        ...
        reduced_metrics = {
            key: self.strategy.reduce(
                ...
                value,
                axis=None
            ) if key != 'generated_images' else value
    ...
    def train(self):
        ...
        for epoch in range(self.initial_epoch, self.epochs):
            ...
            for step, inputs in enumerate(train_dist):
                try:
                    # Get the real images from inputs before running the step
                    (real_sar, _), (real_optical, _) = inputs
                    
                    # Run training step
                    step_metrics = self.distributed_train_step(inputs)
                    
                    # Store generated images if available
                    if 'generated_images' in step_metrics:
                        fake_optical, fake_sar, cycled_optical, cycled_sar = step_metrics['generated_images']
                        ...
                        # Update metrics
                        epoch_metrics['psnr'].append(psnr_optical.numpy())
                        epoch_metrics['ssim'].append(ssim_optical.numpy())
                    
                    # Update other metrics
                    for key in ['gen_loss', 'disc_loss', 'cycle_loss', 'cls_loss']:
                        if key in step_metrics:
                            epoch_metrics[key].append(step_metrics[key].numpy())

                    # Print progress every 100 steps
                    if step % 100 == 0:
                        current_metrics = {
                            k: np.mean(v) for k, v in epoch_metrics.items() if len(v) > 0
                        }
                        metrics_str = " - ".join([
            ...
            # Print epoch summary
            print(f"\n{'='*80}")
            print(f"Epoch {epoch+1} Summary - Time: {time.time() - start_time:.2f}s")
            for metric_name, value in epoch_avg_metrics.items():
