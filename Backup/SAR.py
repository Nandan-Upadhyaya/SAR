...
class Generator(tf.keras.Model):
    ...
    def _build_bottleneck(self) -> tf.keras.Sequential:
        ...
        layers = []
        for _ in range(6):  # Increased residual blocks for better feature extraction
            layers.append(ResidualBlock(256, initializer))
...
class CycleGAN:
    ...
    # Get the batch size dynamically from the prediction tensor
     ...
     if target_is_real:
        ...
        target = tf.ones_like(pred) * 0.9  # Use 0.9 instead of random for stability
     else:
        # Create target tensor with dynamic shape
        target = tf.zeros_like(pred) * 0.1  # Use 0.1 instead of random for stability
    ...
    def _classification_loss(
        ...
        return tf.reduce_mean(tf.keras.losses.categorical_crossentropy(
            labels, pred_cls, label_smoothing=0.1
        ))
